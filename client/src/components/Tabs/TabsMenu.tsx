...
export default function BasicTabsResult({
	...
	const isCheckoutResult = useSelector(
		(state: any) => state.generalStore.isCheckoutResult
	);
	...
	React.useEffect(() => {
		...
		const listeningParts = [
			testBankData.listening.part1[RES_DATA].questions[RES_DATA]
				.subQuestion,
			testBankData.listening.part2[RES_DATA].questions[RES_DATA]
				.subQuestion,
			testBankData.listening.part3[RES_DATA].questions[RES_DATA]
				.subQuestion,
		...
		if (currentExamPart !== 'result' || !isCheckoutResult) return;
		...
		const objectsToSave = {
			...
			coreListening,
			coreReading,
			dateTest: dayjs(new Date()).format('DD/MM/YYYY'),
		};

		saveObjectToDB(objectsToSave);
		dispatch(SET_IS_CHECKOUT_RESULT(false));
				...
				<div className="flex gap-2">
					{currentExamPart !== 'result' && (
						<>
							{isShowResult && (
								<div
									className="w-fit p-1 px-4 bg-[#e8f2ff] rounded-xl font-sans text-[#35509a] font-medium  cursor-pointer hover:bg-blue-200"
					...
					{currentExamPart === 'result' && (
						<>
							<div
								className="w-fit p-1 px-4 bg-gray-100 rounded-xl font-sans font-medium cursor-pointer hover:bg-blue-200"
								onClick={() =>
									dispatch(SET_IS_SHOW_OVERALL_SCORE(true))
								}
							>
								Tổng quát
							</div>
							<div
								className="w-fit p-1 px-4 bg-gray-100 rounded-xl font-sans font-medium cursor-pointer hover:bg-blue-200"
								onClick={() =>
									dispatch(SET_IS_SHOW_OVERALL_SCORE(false))
								}
							>
								Chi tiết
							</div>
